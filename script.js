class DigicalPro {
    ...
    generateShareContent() {
        ...
        this.rooms.forEach((room, index) => {
            ...
            content += `${'-'.repeat(40)}\n`;
            content += `Dimensions: ${parseFloat(room.length) || 0}' × ${parseFloat(room.width) || 0}' × ${parseFloat(room.height) || 0}'\n\n`;
            
            // Wall Area Calculations
            content += `WALL AREA CALCULATIONS:\n`;
            content += `• Wall 1 & 3: ${parseFloat(room.length) || 0}' × ${parseFloat(room.height) || 0}' = ${areas.walls.wall1.toFixed(2)} sq ft each\n`;
            content += `• Wall 2 & 4: ${parseFloat(room.width) || 0}' × ${parseFloat(room.height) || 0}' = ${areas.walls.wall2.toFixed(2)} sq ft each\n`;
            ...
            // Other Areas
            content += `OTHER AREAS:\n`;
            content += `• Ceiling Area: ${parseFloat(room.length) || 0}' × ${parseFloat(room.width) || 0}' = ${areas.ceiling.toFixed(2)} sq ft\n`;
            content += `• Floor Area: ${parseFloat(room.length) || 0}' × ${parseFloat(room.width) || 0}' = ${areas.floor.toFixed(2)} sq ft\n`;
            ...
            if (costs.wall > 0) {
                content += `• Wall Cost: ${areas.walls.net.toFixed(2)} sq ft × ₹${parseFloat(room.pricing.wallPrice) || 0}/sq ft = ₹${costs.wall.toLocaleString('en-IN', {maximumFractionDigits: 2})}\n`;
            }
            if (costs.ceiling > 0) {
                content += `• Ceiling Cost: ${areas.ceiling.toFixed(2)} sq ft × ₹${parseFloat(room.pricing.ceilingPrice) || 0}/sq ft = ₹${costs.ceiling.toLocaleString('en-IN', {maximumFractionDigits: 2})}\n`;
            }
            if (costs.floor > 0) {
                content += `• Floor Cost: ${areas.floor.toFixed(2)} sq ft × ₹${parseFloat(room.pricing.floorPrice) || 0}/sq ft = ₹${costs.floor.toLocaleString('en-IN', {maximumFractionDigits: 2})}\n`;
            }
            if (costs.doors > 0) {
                content += `• Doors Cost: ${areas.acousticDoors} units × ₹${parseFloat(room.pricing.doorPrice) || 0}/unit = ₹${costs.doors.toLocaleString('en-IN', {maximumFractionDigits: 2})}\n`;
            }
            if (costs.additional > 0) {
                content += `• Additional Areas Cost: ${areas.additional.toFixed(2)} sq ft × ₹${parseFloat(room.pricing.additionalPrice) || 0}/sq ft = ₹${costs.additional.toLocaleString('en-IN', {maximumFractionDigits: 2})}\n`;
            }
            content += `\n`;
